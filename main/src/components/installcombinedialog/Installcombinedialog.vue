  ...
  export default {
    ...
    methods: {
      ...
      handleKeyInstall() {
        if (this.isXb && !this.installed && !this.installing) {
          ...
          chrome.xb.getAllExtensions ((res)=>{
            ...
            res.map((item)=>{ this.installedIDData_.push(item.id); });
            
            for (var i = 0, len = plug_list.length; len > i; i++) {
              if (this.installedIDData_.indexOf(plug_list[i].appid) === -1) {
                this.installingData.push(plug_list[i].appid);
                ...
                if(checkBrowserVersion([10,1,2713,1466])){
                  ...
                  chrome.xb.switchToExtensionManager(plug_list[i].app_url, 1)
                }
                // 
                
              }
            }
            // if(this.detail._id === '284lpujuyolg'){
            //   console.log('__11',this.once,this.stapNum,this.stopCount,this.notInstallData,this.installedIDData,this.installingData)
            // }
          });
      ...
      q() {
        ...
        //this.stopCount = this.randomStopNum + this.stapNum;
        
        // console.log('q',this.stopCount,this.stapNum)
        this.timer_q = setInterval(()=>{
          this.qFn()
        }, 4) //time *  500 = 2.5, 50 * 500 = 25000
      },
      qFn() {
        // console.log('count',this.count,this.stopCount)
        let randomNum = .6 * Math.random();// 100 - this.randomStopNum(50~53) => (47~50)
        this.count += randomNum; // .2 * 500       0 ~ .6
        if (randomNum < .01) {
          clearInterval(this.timer_q);
          clearTimeout(this.timer_out_q);
